 ...
 */
function recursivelyDeserializeBuffers(obj) {
    ...
    if (typeof obj === "object" &&
        ...
        try {
            ...
            logger.warn({ error, objType: typeof obj, hasData: !!obj.data }, "Failed to convert Buffer-like object to Buffer");
            return obj;
        }
    }
    // Handle arrays
...
export class PostgresAuthState {
    ...
    async getAuthState(sessionId) {
        ...
        const result = await this.pool.query("SELECT auth_state FROM whatsapp_sessions WHERE id = $1", [sessionId]);
        ...
        if (result.rows[0]?.auth_state) {
            ...
            if (stored.keys) {
                ...
                keys = recursivelyDeserializeBuffers(keys);
                    ...
                    for (const id of ids) {
                        ...
                        if (keys[key]) {
                                ...
                                if (publicKey && privateKey) {
                                    ...
                                    if (!publicIsBuffer || !privateIsBuffer) {
                                        logger.warn({
                                            ...
                                            publicType: typeof publicKey,
                                            privateType: typeof privateKey,
                                            publicHasType: !!publicKey?.type,
                                            privateHasType: !!privateKey?.type
                                        ...
                                        if (!publicIsBuffer && publicKey?.type === "Buffer") {
                                            value.public = Buffer.from(publicKey.data, "base64");
                                        }
                                        if (!privateIsBuffer && privateKey?.type === "Buffer") {
                                            value.private = Buffer.from(privateKey.data, "base64");
                                        }
                            ...
                            logger.debug({
                                ...
                                preKeyCount: Object.keys(keys).filter((k) => k.startsWith("pre-key-")).length
    ...
    async saveToDb(sessionId, creds, keys) {
        const authStateJson = JSON.stringify({ creds, keys }, BufferJSON.replacer);
        await this.pool.query(`UPDATE whatsapp_sessions
       SET auth_state = $1, updated_at = NOW()
       WHERE id = $2`, [authStateJson, sessionId]);
    }
