 ...
 */
export async function downloadAndUploadMedia(socket, message, sessionId) {
    try {
        ...
        logger.debug({ messageId, base64Length: base64Data.length, uploadUrl: `${env.nextjsAppUrl}/api/whatsapp/media` }, "Uploading media to Next.js as JSON");
        ...
        logger.debug({ messageId, bodyLength: requestBody.length, headers: { "Content-Type": "application/json", "x-api-key": "***" } }, "Sending media upload request");
        ...
        if (!response.ok) {
            ...
            logger.error({ messageId, status: response.status, statusText: response.statusText, error: errorText }, "Failed to upload media to Next.js API");
            return null;
        }
 ...
 */
function getExtension(mimeType, defaultExt) {
 ...
 */
export function hasDownloadableMedia(message) {
    const msg = message.message;
    if (!msg)
        return false;
