...
// Get AI config
router.get("/:sessionId/ai-config", async (req: Request, res: Response, next: NextFunction) => {
  try {
...
// Update AI config
router.put("/:sessionId/ai-config", async (req: Request, res: Response, next: NextFunction) => {
  try {
    const { sessionId } = req.params
...
// Test AI response
router.post("/:sessionId/test-ai", async (req: Request, res: Response, next: NextFunction) => {
  try {
    const { sessionId } = req.params
    ...
    const result = await aiResponder.testResponse(
      testMessage,
