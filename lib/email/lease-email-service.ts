...
export async function sendLeaseToTenantAction(
  ...
  try {
    ...
    const fileName = `lease-${leaseId}-draft.pdf`
...
export async function notifyLandlordTenantSignedAction(
  ...
  try {
    ...
    // Get landlord email - ALWAYS use landlord email, never rental agent email
    // Priority: 1) Property's stored landlord email, 2) Lease extractionData, 3) Landlord record
    let landlordEmail = ""
    ...
    // If still not found, get from property's landlord record
    if (!landlordEmail) {
    ...
    const appUrl = process.env.NEXT_PUBLIC_APP_URL || "http://localhost:3000"
    const signingLink = `${appUrl}/dashboard/leases/${leaseId}/sign`
...
Property Management Team
    ...
    // Send to landlord if email found
    if (landlordEmail) {
      await postmarkClient.sendEmail({
        From: fromEmail,
        To: landlordEmail,
