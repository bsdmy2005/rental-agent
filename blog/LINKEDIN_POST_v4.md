...
## An Energetic Framework That Stands In For You

The real achievement? Building an **energetic framework** that can coordinate, delegate, and effectively stand in for you. This became clear with the WhatsApp automation - one of the most satisfying parts of the system.

Here's what it does: Tenants message about issues - a leak, a broken appliance, whatever needs attention. Instead of you having to respond, coordinate, and make decisions, a network of sub-agents handles it autonomously. One agent receives the message and creates an incident. Another identifies what needs to be done and which service providers to contact. Yet another sends RFQ requests via WhatsApp to your trusted providers. When quotes come back, comparison agents evaluate them and select the best option. Dispatch agents coordinate the work, and payment agents handle the financial side.

The entire workflow runs autonomously. A tenant reports an issue. The agent network creates the incident, identifies requirements, sends RFQ requests to the right service providers, collects quotes, compares them, selects the best option, dispatches the work, and handles payment. All without you having to coordinate messages, compare prices, or remember to follow up.

This is what sub-agent architecture enables: an energetic framework that can coordinate with an unlimited number of parties, make decisions, and execute entire workflows. The agents handle the coordination, the comparison, the execution, and the payment - operating as a true stand-in for you, but at a scale you couldn't achieve manually.
...
## The Future of Banking: Agent-Controlled Accounts with Guardrails
...
This requires thinking around the future of banking - around **special types of accounts controlled by agents with boundaries, built-in guardrails, and intelligent decision-making capabilities**. Imagine an account where agents can make decisions, execute payments, and reconcile transactions - all within guardrails you define. The account becomes intelligent, responding to bills automatically, matching payments to invoices, and maintaining perfect records.

**Shoutout to Investec** for making their Open Banking platform accessible to end users rather than hiding it behind crazy abstractions. This accessibility is what enabled me to build these autonomous payment workflows. More banks need to start thinking about creating these accounts and making them accessible - not just for developers, but for end users who want to build intelligent financial automation.
...
If you're interested in engaging about this, feel free to reach out to me. I'm always interested in topics around AI agents, banking, and finance.
