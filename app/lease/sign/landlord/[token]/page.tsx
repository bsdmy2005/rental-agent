...
import { getLeaseByLandlordTokenAction } from "@/actions/lease-initiation-actions"
...
export default function LandlordSigningPage() {
  ...
  useEffect(() => {
    async function loadLease() {
      try {
        const result = await getLeaseByLandlordTokenAction(token)
        if (!result.isSuccess) {
  ...
  const handleSign = async () => {
    ...
    try {
      const response = await fetch(`/api/leases/${lease.id}/sign/landlord/token`, {
        method: "POST",
        headers: {
      ...
      router.push(`/lease/sign/landlord/${token}/success`)
    } catch (err) {
          ...
          <CardHeader>
            ...
            <CardDescription className="space-y-2 mt-2">
              ...
              <p className="text-sm text-muted-foreground mt-4">
                Please use the signing link provided in your email. If you don't have the email, please contact your rental agent or property management team.
              </p>
            </CardDescription>
          </CardHeader>
        </Card>
    ...
    <div className="container mx-auto py-8 max-w-4xl">
      <Card>
        <CardHeader>
          ...
          <CardDescription>
            Please review the lease agreement and provide your signature below. Once you sign, the lease will be sent to the tenant for their signature.
          </CardDescription>
        </CardHeader>
