...
import { uploadExpenseAttachmentAction } from "@/actions/expenses-actions"
  ...
  try {
    ...
    if (!file || !expenseId) {
      return NextResponse.json(
        { error: "Missing required fields: file, expenseId" },
        { status: 400 }
      )
    }
    ...
    const filePath = `expenses/${expenseId}/${timestamp}-${sanitizedFileName}`
    ...
    // Create expense attachment record
    const fileType = isPDF ? "pdf" : "image"
    const result = await uploadExpenseAttachmentAction({
      expenseId,
      fileUrl,
      fileName: file.name,
      fileType
